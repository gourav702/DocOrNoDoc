...
# processed_dataset.save_to_disk("Dataset/processed_dataset_fp16")
...
# 
# # Check for MPS (Mac) and disable FP16 if detected, as it's unstable
# use_fp16 = torch.cuda.is_available() # Only use FP16 for NVIDIA GPUs
# 
# training_args = TrainingArguments(
#     output_dir="document_classifier_model_fp16",
#     learning_rate=2e-5,
...
#     fp16=use_fp16, # Dynamically set FP16
#     push_to_hub=False,
#     report_to="none"
# )
# 
...
HF_REVISION = "fp16" # The branch name for our 250MB model
MODEL_URL = f"https://huggingface.co/{HF_REPO_ID}/tree/{HF_REVISION}"

reader = easyocr.Reader(['en'])

print(f"🧠 Loading lightweight (250MB) model from Hugging Face Hub...")
print(f"   URL: {MODEL_URL}")

try:
    # Determine the device to use (MPS for Mac, CUDA for NVIDIA, CPU otherwise)
    device = "mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu"
    print(f"   Using device: {device}")

    # Load the specific FP16 version from the Hub
    model = LayoutLMv3ForSequenceClassification.from_pretrained(HF_REPO_ID, revision=HF_REVISION).to(device)
    processor = LayoutLMv3Processor.from_pretrained(HF_REPO_ID, revision=HF_REVISION)
    print("✅ Successfully loaded FP16 model from the Hub.")
except Exception as e:
    print(f"❌ Failed to load model from the Hub: {e}")
    print("   Please make sure you have run 'convert_to_fp16.py' successfully first.")
    exit(1)
...
for filename in sorted(os.listdir(test_folder)):
    ...
    if filename.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.tif')):
        try:
            ...
            # THE FIX: Ensure the pixel_values tensor is converted to the model's dtype (half)
