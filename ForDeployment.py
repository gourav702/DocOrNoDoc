...
checkpoints = glob.glob("./deployment_model/checkpoint-*")

if not checkpoints:
    print("❌ No trained model found! Please uncomment the Training section and run it first.")
    exit(1)

best_model_path = max(checkpoints, key=os.path.getctime)
print(f"🧠 Loading Brain from: {best_model_path}")
